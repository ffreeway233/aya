...
function Init(v1,v2,v3) {
    //打开xlmdocm文档
    xmlDoc = loadXmlFile('../static/common/Area2.xml');
    ...
    if (TopnodeList.length > 0) {
        ...
        if (TopnodeList.length > 0) {
            ...
            if (province .length> 0) {
                //县列表
                var countys = province[childnodeindex].getElementsByTagName("county");
                for (var i = 0; i < countys.length; i++) {
                    var id = dropElement2.options[childnodeindex].value;
                    //默认为第一个城市的第一个县列表
